...
/*
	...
	@program description: red-black search tree
 */
...
public class Tree
{
    ...
    public Object find (int k) {
		...
		while (currentNode != nil) {
			if (k == currentNode.key) return currentNode.item;
    ...
    private static Node insert (Node v, int k, Object x) {
		// base case: if we hit the "nil", create a new RED node
		if (v == nil) {
			return Node.allocate(k, x, nil, nil, RED);
		}

		// BST recursion: find where k belongs
		if (k < v.key) {
			// go left of the tree
			...
			// k == v.key: update the item
			v.item = x;
		}

		// restore red-black properties at this subtree
		return insertRestore(v, k);
